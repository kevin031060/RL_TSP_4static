...
def train(actor, critic, w1, w2, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    save_dir = os.path.join(task, '%d' % num_nodes, 'w_%2.2f_%2.2f' % (w1, w2), now)
    ...
    for epoch in range(1):
        ...
        if mean_valid < best_reward:
            ...
            main_dir = os.path.join(task, '%d' % num_nodes, 'w_%2.2f_%2.2f' % (w1, w2))
...
if __name__ == '__main__':
    ...
    if args.task == 'tsp':
        w2_list = np.arange(51)/50
        for i in range(0,51):
            print("Current w:%2.2f/%2.2f"% (1-w2_list[i], w2_list[i]))
            if i == 0:
                train_tsp(args, 1, 0, "tsp/20/w_1.00_0.00")
            else:
                checkpoint = 'tsp/20/w_%2.2f_%2.2f' % (1-w2_list[i-1], w2_list[i-1])
